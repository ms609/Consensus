...
This implementation ports the near-linear \\O(kn \log n)\\ algorithm of
Jansson et al. (2024) from their FDCT reference C++ (used with
permission): cluster frequencies are computed by a divide-and-conquer
labelling with radix sort, and conflicting lower-frequency clusters are
filtered out using centroid-path decomposition, lowest-common-ancestor
and range-minimum queries, and tree contraction, avoiding the explicit
\\O(s^2)\\ pairwise compatibility matrix used previously.
