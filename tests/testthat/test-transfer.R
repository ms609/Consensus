test_that("Identical trees return fully resolved consensus", {
  ...
  tc <- Transfer(trees, greedy = "first")
  expect_equal(NSplits(tc), NSplits(tree))

  tc_best <- Transfer(trees, greedy = "best")
...
test_that("Both greedy strategies produce valid trees", {
  ...
  tc_first <- Transfer(trees, greedy = "first")
  tc_best  <- Transfer(trees, greedy = "best")
...
test_that("scale = FALSE (unscaled) works", {
  ...
  tc_scaled <- Transfer(trees, scale = TRUE)
  tc_unscaled <- Transfer(trees, scale = FALSE)
...
test_that("init = 'majority' works", {
  ...
  tc_empty <- Transfer(trees, init = "empty")
  tc_maj   <- Transfer(trees, init = "majority")
...
test_that("Error on bad input", {
  expect_error(Transfer(list(TreeTools::BalancedTree(5))), "multiPhylo")
  expect_error(Transfer(
  ...
  tiny <- structure(rep(list(ape::read.tree(text = "(a,b,c);")), 3),
                    class = "multiPhylo")
  tc_tiny <- Transfer(tiny)
  expect_equal(NSplits(tc_tiny), 0)

  # tc_profile validation
  expect_error(ConsTree:::tc_profile(list(TreeTools::BalancedTree(5))),
               "multiPhylo")
  expect_error(ConsTree:::tc_profile(
    structure(list(TreeTools::BalancedTree(5)), class = "multiPhylo")),
    "at least 2")
...
test_that("Transfer returns star tree for all-star input", {
  star <- StarTree(8)
...
test_that("Transfer covers all parameter combinations", {
  trees <- as.phylo(0:9, nTip = 10)

  # greedy="first", init="majority", scale=FALSE
  tc <- Transfer(trees, greedy = "first", init = "majority",
                 scale = FALSE)
  expect_s3_class(tc, "phylo")

  # greedy="best", init="majority", scale=FALSE
  tc2 <- Transfer(trees, greedy = "best", init = "majority",
                  scale = FALSE)
  expect_s3_class(tc2, "phylo")

  # greedy="first", init="empty", scale=FALSE
  tc3 <- Transfer(trees, greedy = "first", init = "empty",
                  scale = FALSE)
...
test_that("Greedy remove path fires with conflicting majority splits", {
  ...
  tc_best_s <- Transfer(trees, greedy = "best", init = "majority",
                        scale = TRUE)
  expect_s3_class(tc_best_s, "phylo")

  tc_best_u <- Transfer(trees, greedy = "best", init = "majority",
                        scale = FALSE)
  expect_s3_class(tc_best_u, "phylo")

  # greedy="first" exercises the same remove path via greedy_first

  tc_first_s <- Transfer(trees, greedy = "first", init = "majority",
                         scale = TRUE)
  expect_s3_class(tc_first_s, "phylo")

  tc_first_u <- Transfer(trees, greedy = "first", init = "majority",
                         scale = FALSE)
    ...
    for (gr in c("best", "first")) {
      for (init in c("empty", "majority")) {
        for (sc in c(TRUE, FALSE)) {
          tc <- Transfer(trees, greedy = gr, init = init, scale = sc)
          expect_s3_class(tc, "phylo")
        }
      }
    }
...
test_that("tc_profile() runs without error", {
  ...
  res <- ConsTree:::tc_profile(trees, scale = TRUE, greedy = "best",
  ...
  res2 <- ConsTree:::tc_profile(trees, scale = FALSE, greedy = "first",
                                init = "majority")
  expect_length(res2, 5)
  expect_true(all(res2 >= 0))

  # Multiple iterations
  res3 <- ConsTree:::tc_profile(trees, n_iter = 3L)
  expect_length(res3, 5)

  # Greedy best, majority init
  res4 <- ConsTree:::tc_profile(trees, init = "majority")
  expect_length(res4, 5)
})


# =========================================================================
# R-level internal helper functions
# =========================================================================

test_that(".FlipRaw() flips bits and masks correctly", {
  # 8 tips (1 byte, all bits used, mask = 0xFF)
  expect_equal(ConsTree:::.FlipRaw(as.raw(0x07), 8), as.raw(0xf8))

  # 5 tips (1 byte, 5 bits, mask = 0x1F)
  expect_equal(ConsTree:::.FlipRaw(as.raw(0x07), 5), as.raw(0x18))

  # Multi-byte: 10 tips (2 bytes, last byte 2 bits, mask = 0x03)
  result <- ConsTree:::.FlipRaw(as.raw(c(0x00, 0x01)), 10)
...
test_that(".TransferDistMat() computes pairwise transfer distances", {
  ...
  DIST <- ConsTree:::.TransferDistMat(splitMat, 6)
  ...
  # Single split
  DIST1 <- ConsTree:::.TransferDistMat(
...
test_that(".CompatMat() checks bipartition compatibility", {
  ...
  compat <- ConsTree:::.CompatMat(splitMat, 6)
...
test_that(".ComputeTD() works for both scale modes", {
  ...
  td_s <- ConsTree:::.ComputeTD(DIST, sentDist, treeMembers, lightSide, 2, TRUE)
  td_u <- ConsTree:::.ComputeTD(DIST, sentDist, treeMembers, lightSide, 2, FALSE)
...
test_that(".IsCompat() handles edge cases", {
  ...
  expect_true(ConsTree:::.IsCompat(1, c(FALSE, FALSE, FALSE), compat, 10))
  # Too many included (>= nTip - 3) → never compatible
  expect_false(ConsTree:::.IsCompat(1, c(TRUE, TRUE, TRUE), compat, 6))
  # Compatible with included set
  expect_true(ConsTree:::.IsCompat(1, c(FALSE, TRUE, FALSE), compat, 10))
  # Incompatible with an included split
  expect_false(ConsTree:::.IsCompat(1, c(FALSE, FALSE, TRUE), compat, 10))
})
...
test_that(".Dist() returns correct distance", {
  ...
  expect_equal(ConsTree:::.Dist(1, 2, DIST, sentDist), 2)
  expect_equal(ConsTree:::.Dist(1, NA, DIST, sentDist), 5)
  expect_equal(ConsTree:::.Dist(3, NA, DIST, sentDist), 7)
})
...
test_that(".DiagDist() vectorizes correctly", {
  ...
  expect_equal(
    ConsTree:::.DiagDist(c(2, NA, 1), DIST, sentDist),
    c(DIST[1, 2], 6, DIST[3, 1])
  )
  expect_equal(ConsTree:::.DiagDist(c(NA, NA, NA), DIST, sentDist), sentDist)
  expect_equal(
    ConsTree:::.DiagDist(c(2, 3, 1), DIST, sentDist),
...
test_that(".FindSecond() works for both scale modes and edge cases", {
  ...
  expect_equal(
    ConsTree:::.FindSecond(1, 2, c(1, 2, 3), DIST, pMinus1, FALSE), 1)

  # matchIdx = NA → search all
  expect_equal(
    ConsTree:::.FindSecond(1, NA_integer_, c(2, 3), DIST, pMinus1, FALSE), 2)

  # No candidates
  expect_true(is.na(
    ConsTree:::.FindSecond(1, 2, c(2), DIST, pMinus1, FALSE)))

  # All exceed threshold (unscaled)
  DIST_far <- matrix(c(0, 5, 5, 5, 0, 5, 5, 5, 0), 3, 3)
  expect_true(is.na(
    ConsTree:::.FindSecond(1, NA_integer_, c(2, 3), DIST_far, c(2, 2, 2), FALSE)
  ))

  # Scaled: normal case
  expect_equal(
    ConsTree:::.FindSecond(1, NA_integer_, c(2, 3), DIST, pMinus1, TRUE), 2)

  # Scaled: exceeds threshold
  expect_true(is.na(
    ConsTree:::.FindSecond(1, NA_integer_, c(2), DIST_far, c(2, 2, 2), TRUE)
  ))
})
...
test_that(".DoAdd() updates match state correctly", {
  ...
  ConsTree:::.DoAdd(2, st, DIST, sentDist)
  ...
  ConsTree:::.DoAdd(1, st, DIST, sentDist)
...
test_that(".DoRemove() handles sentinel promotion and affected2", {
  ...
  ConsTree:::.DoRemove(2, st, DIST, sentDist, lightSide, FALSE)
  ...
  ConsTree:::.DoRemove(2, st2, DIST, sentDist, lightSide, FALSE)
  ...
  ConsTree:::.DoRemove(2, st3, DIST, sentDist, lightSide, FALSE)
  ...
  ConsTree:::.DoRemove(2, st4, DIST, sentDist, lightSide, TRUE)
...
test_that(".RemoveBenefitVec() computes remove benefits", {
  ...
  ben_none <- ConsTree:::.RemoveBenefitVec(
...
test_that(".SplitsToPhylo() converts splits to tree or star", {
  ...
  tr <- ConsTree:::.SplitsToPhylo(pool$rawSplits, included, tipLabels, 8)
  expect_s3_class(tr, "phylo")
  expect_equal(sort(TipLabels(tr)), sort(tipLabels))

  # No splits → star tree
  tr_star <- ConsTree:::.SplitsToPhylo(
...
test_that(".InitMatches() initializes match state from included splits", {
  ...
  ConsTree:::.InitMatches(st, DIST, sentDist, lightSide, scale = TRUE)
  ...
  ConsTree:::.InitMatches(st2, DIST, sentDist, lightSide, scale = FALSE)
  ...
  ConsTree:::.InitMatches(st3, DIST, sentDist, lightSide, scale = TRUE)
...
test_that("R-level GreedyBest pipeline produces valid consensus (scaled)", {
  ...
  tr <- ConsTree:::.SplitsToPhylo(pool$rawSplits, st$incl, tipLabels, nTip)
  expect_s3_class(tr, "phylo")
})
...
test_that("R-level GreedyFirst pipeline produces valid consensus (unscaled)", {
  ...
  ConsTree:::.GreedyFirst(st, DIST, sentDist, TD, pool$counts, pool$lightSide,
...
test_that("R-level GreedyBest with majority init exercises InitMatches", {
  ...
  if (any(st$incl)) {
    ConsTree:::.InitMatches(st, DIST, sentDist, pool$lightSide, scale = TRUE)
  }
...
test_that("R-level GreedyFirst with majority init (unscaled)", {
  ...
  st$incl <- pool$counts > (nTree / 2)
  if (any(st$incl)) {
    ConsTree:::.InitMatches(st, DIST, sentDist, pool$lightSide, scale = FALSE)
  }
