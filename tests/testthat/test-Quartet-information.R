# Net concordant quartet information optimized by Quartet():
...
test_that("penalty must be a single non-negative number", {
  ...
  expect_error(Quartet(trees, penalty = -1), "non-negative")
  expect_error(Quartet(trees, penalty = c(1, 2)), "single")
  expect_error(Quartet(trees, penalty = NA_real_), "non-negative")
  expect_error(Quartet(trees, penalty = "1"), "non-negative")
})
...
test_that("penalty controls resolution: low resolves more than high", {
  ...
  # near-unanimous ones.
  qc_lenient <- Quartet(trees, penalty = 0)
  qc_strict <- Quartet(trees, penalty = 50)
...
test_that("clean data is never pruned and stays fully resolved", {
  ...
  qc <- Quartet(trees, neverDrop = FALSE)
