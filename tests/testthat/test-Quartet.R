test_that("Quartet() handles bad-sized trees", {
  expect_error(Quartet(c(TreeTools::BalancedTree(3),
                                  TreeTools::BalancedTree(3))),
               "Need at least 4 tips")
  expect_error(Quartet(c(TreeTools::BalancedTree(103),
...
test_that("greedy=first adds splits from empty start", {
  ...
  qc <- Quartet(trees, init = "empty", greedy = "first")
...
test_that("greedy=first prunes harmful splits from extended init", {
  ...
  qc_ext <- Quartet(trees, init = "extended", greedy = "first")
  qc_maj <- Quartet(trees, init = "majority", greedy = "first")
...
test_that("Quartet minimizes quartet distance", {
  ...
  qd_qc <- sum(vapply(trees, function(tr) {
    s <- Quartet::QuartetStatus(qc, tr)
    2 * s[, "d"] + s[, "r1"] + s[, "r2"]
  }, numeric(1)))

  qd_mr <- sum(vapply(trees, function(tr) {
    s <- Quartet::QuartetStatus(mr, tr)
    2 * s[, "d"] + s[, "r1"] + s[, "r2"]
  }, numeric(1)))
...
test_that("C++ guards reject invalid n_tips", {
  ...
  expect_error(
    Consensus:::cpp_quartet_consensus(list(), 101, TRUE, FALSE, TRUE, NULL),
    "100"
  )
  expect_error(
    Consensus:::cpp_quartet_consensus(list(), 3, TRUE, FALSE, TRUE, NULL),
    "4 tips"
  )
})
...
test_that("Quartet is deterministic", {
  ...
  status <- Quartet::QuartetStatus(qc1, qc2)
  expect_equal(unname(status[, "d"]), 0)
  expect_equal(unname(status[, "r1"]), 0)
  expect_equal(unname(status[, "r2"]), 0)
})
...
test_that("Quartet brute-force verification (n=5)", {
  ...
  qd_all <- vapply(seq_along(all_trees), function(i) {
    ...
    sum(vapply(trees, function(tr) {
      s <- Quartet::QuartetStatus(cand, tr)
      2 * s[, "d"] + s[, "r1"] + s[, "r2"]
    }, numeric(1)))
  }, numeric(1))
  ...
  qd_star <- sum(vapply(trees, function(tr) {
    s <- Quartet::QuartetStatus(star, tr)
    2 * s[, "d"] + s[, "r1"] + s[, "r2"]
  }, numeric(1)))
